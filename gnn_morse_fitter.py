...
from gnn_morse_local.data import DFTDataset
from gnn_morse_local.models import GNNMorseModel
from gnn_morse_local.lammps_export import export_lammps
from gnn_morse_local.plotting import generate_all_plots
...
def main():
    ...
    for i, arg in enumerate(sys.argv):
        if arg == '--output-dir' and i + 1 < len(sys.argv):
            output_base = os.path.abspath(sys.argv[i + 1])
            break

    if output_base is None:
        # Default: outputs go next to the config file
        output_base = os.path.dirname(os.path.abspath(sys.argv[1]))

    os.makedirs(output_base, exist_ok=True)

    log_path = os.path.join(output_base, 'training.log')
    ...
    # ── Save checkpoint ──
    ckpt_path = os.path.join(output_base, 'checkpoint.pt')
    torch.save({
        'epoch': epoch,
