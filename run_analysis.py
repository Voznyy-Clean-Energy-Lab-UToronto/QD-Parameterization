...
from gnn_morse_local.utils import base_element, FROZEN_LJ
...
def run_force_rerun(run_dir, dft_xyz, core_elements, results_dir, lammps_bin='lmp'):
    """Feed DFT positions through LAMMPS potential, compare forces."""
    import ase.io

    mapping_file = os.path.join(run_dir, 'atom_type_mapping.txt')
    data_file = os.path.join(run_dir, 'gnn_morse.data')
    potentials_file = os.path.join(run_dir, 'potentials.txt')
    lammps_in = os.path.join(run_dir, 'gnn_morse.in')
...
def main():
    ...
    parser.add_argument('--lammps-bin',
                        default='/Users/abiciunas/miniconda3/envs/lammps-env/bin/lmp')
    ...
    if os.path.exists(config_file):
        ...
        if datasets:
            ...
            dft_xyz = os.path.normpath(os.path.join(run_dir, xyz_rel))
        od = config.get('original_data', '')
        if od:
            original_data = os.path.normpath(os.path.join(run_dir, od))
    ...
    lammps_in = os.path.join(run_dir, 'gnn_morse.in')
    md_xyz = os.path.join(run_dir, 'gnn_morse.xyz')
    md_log = os.path.join(run_dir, 'gnn_morse.log')
    ...
    if not args.skip_md:
        ...
        if not os.path.exists(lammps_in):
            ...
            print("  Run the fitter first: python -m gnn_morse_local config.yaml")
            return
        print(f"\n  Running GNN-Morse MD ({args.nsteps} steps at {args.temp}K)...")
        try:
            result = subprocess.run(
                [args.lammps_bin, '-in', 'gnn_morse.in'],
                cwd=run_dir, capture_output=True, text=True, timeout=3600)
        except FileNotFoundError:
            print(f"  [Error] LAMMPS binary '{args.lammps_bin}' not found.")
            return
        if result.returncode != 0:
        ...
        # Infante MD
        if not args.no_infante:
            infante_xyz, infante_log = run_infante_md(
                original_data, results_dir,
                temp=args.temp, nsteps=args.nsteps, lammps_bin=args.lammps_bin)
    else:
        print(f"\n  Skipping MD (--skip-md)")
    ...
    if not args.skip_rerun:
        ...
        print(f"{'─'*60}")
        if dft_xyz:
            run_force_rerun(run_dir, dft_xyz, core_elements, results_dir,
